...
(function () {
  ...
  function scriptableWidgetCode(token) {
    ...
    return [
      ...
      "  if (raw && raw.trim() !== 'null') data = JSON.parse(raw);",
      ...
      "  const picker = new Alert(); picker.title = 'Work Payment Log'; picker.message = 'Choose a widget size to preview';",
      "  picker.addAction('Small square'); picker.addAction('Medium'); picker.addAction('Large');",
      "  const choice = await picker.presentSheet(); family = choice === 1 ? 'medium' : choice === 2 ? 'large' : 'small';",
      "}",
      "const accentHex = data && /^#[0-9a-f]{6}$/i.test(data.accent || '') ? data.accent : '#ff9147';",
      "const accent = new Color(accentHex);",
      "const dark = !data || data.dark !== false;",
      ...
      "function value(stack, text, size, colour) { const t=stack.addText(String(text)); t.font=Font.boldSystemFont(size || 20); t.textColor=colour || (dark?Color.white():new Color('#24150d')); t.minimumScaleFactor=0.68; t.lineLimit=1; return t; }",
      "function money(n) { return ((data && data.currency) || '€') + Number(n || 0).toFixed(2); }",
      "function shortDate(raw) { if(!raw) return 'Not set'; const parts=String(raw).split('-'); const months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']; const month=months[Number(parts[1])-1]; return month ? Number(parts[2])+' '+month : String(raw); }",
      "",
      "if (!data || !data.summary) {",
      "  value(w, 'Work Payment Log', 15, accent); w.addSpacer(8); value(w, 'Widget not connected', 17); w.addSpacer(4); label(w, loadError || 'Open the web app and refresh widget data in Settings.');",
      "} else if (family === 'large') {",
      "  const head=w.addStack(); head.centerAlignContent(); value(head, 'Work Payment Log', 16, accent); head.addSpacer(); label(head, shortDate(data.summary.paydayDate));",
      "  w.addSpacer(16); label(w,'ESTIMATED TAKE-HOME'); value(w,money(data.summary.takeHome),34,accent);",
      "  w.addSpacer(16); const first=w.addStack(); first.layoutHorizontally();",
      "  const total=first.addStack(); total.layoutVertically(); label(total,'TOTAL PAY'); value(total,money(data.summary.totalPay),20);",
      "  first.addSpacer(); const gross=first.addStack(); gross.layoutVertically(); label(gross,'NEXT PAYDAY'); value(gross,money(data.summary.paydayGross),20);",
      "  w.addSpacer(16); const second=w.addStack(); second.layoutHorizontally();",
      "  const hours=second.addStack(); hours.layoutVertically(); label(hours,'HOURS LOGGED'); value(hours,Number(data.summary.totalHours||0).toFixed(2),20);",
      "  second.addSpacer(); const days=second.addStack(); days.layoutVertically(); label(days,'DAYS WORKED'); value(days,Number(data.summary.daysWorked||0),20);",
      "  w.addSpacer(); label(w,'Estimated deductions '+money(data.summary.deductions));",
      ...
      "}",
      "",
      "if (config.runsInWidget) Script.setWidget(w); else if (family === 'large') await w.presentLarge(); else if (family === 'medium') await w.presentMedium(); else await w.presentSmall();",
      "Script.complete();"
    ].join("\n");
  }
